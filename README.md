# TIL
<h3>Today I Learn 1차</h3>
1장. 코딩 안내서 기초편
2023. 01. 14 

<li>오늘 TIL 3줄 요약</li>

수학능력은 필수조건이 아니다.
암기능력보다 개발 과정을 이해하는 것이 중요하다.
새로운 프로그래밍 언어를 공부하면 그 언어에 담긴 철학, 코드 구조, 개념, 패턴을 이해하고 녹여내는것이 빠른 성장의 길이다.

<li>책에서 기억하고 싶은 내용을 써보세요.</li>

빠른 공부법
1. 공식 문서를 보고 언어를 만든 사람의 철학을 확인 할 것.
2. 공식 문서를 본 뒤에 문법을 공부 할 것
3. 내가 알고 있는 언어와 새 언어나 얼마나 비슷한지 살펴볼것.
4. 최대한 빨리 코드를 한줄이라도 작성해 볼것.
5. 공식 문서에 있는 튜토리얼을 시작할것.

<li>오늘 읽은 소감은? 떠오르는 생각을 가볍게 적어보세요</li>
무작정 문법만 읽고 배우려 했는데 프로그래밍 언어를 배울때 가장 첫번째로 할 것인 공식문서를 확인해야겠다.

<li>궁금한 내용이 있거나, 잘 이해되지 않는 내용이 있다면 적어보세요.</li>
c, c++, c# 자세히는 모르겠다.

---
<h3>Today I Learn 2차</h3>
1장. 코딩 안내서 기초편
2023. 01. 15

<li>오늘 TIL 3줄 요약</li>

실수는 그냥 넘기지 않고 성장을 위한 발판으로 사용하기.
오류날때 책이나 문서탓하지말고 "코드 입력한 나 자신 때문이다" 생각하기. 
오류 고치기 전 "오류 메시지" 읽고 오류 왜 생겼는지 알아내기.

<li>책에서 기억하고 싶은 내용을 써보세요.</li>

[프로그래밍 초보자가 하기 쉬운 실수]
공부만 하고 만들어보지 않는다.
겁나 쉬운것부터 시작해서 나만의 코드로 프로젝트 완성하는 것을 목표로 해야한다. 
뭐든지 일단 만들고 눈앞에 결과물을 보고 자신감 올리기.

<li>오늘 읽은 소감은? 떠오르는 생각을 가볍게 적어보세요</li>
파이썬과 자바스크립트는 배우면 활용하기 무궁무진한 언어이다.

<li>궁금한 내용이 있거나, 잘 이해되지 않는 내용이 있다면 적어보세요.</li>
코틀린...백 파트는 아직 알고싶지 않다ㅎ

---

<h3>Today I Learn 3차</h3>
2장. 코딩별 안내서 - 웹기술편
2023. 01. 17

<li>오늘 TIL 3줄 요약</li>

제이쿼리는 정말 필요할떄만 배워라.
프로그래밍도 운동과 비슷하다. 매일 꾸준히 하는것이 좋다. 
<strong>(공부 체크리스트를 만들지 말고) 무엇을 만들지 프로젝트 체크리스트를 작성해야한다.</strong>
목표와 기한을 설정하여 압박을 주는게 좋다. 인간은 편하고 싶어하는 본성이 있어서 속절없이 시간을 흘려보내기 쉽다.

<li>책에서 기억하고 싶은 내용을 써보세요.</li>

<strong>라이브러리란?</strong>
도구에 대해 모든 결정을 다 내리고 있다. 
우리가 제어할 수 있다.

<strong>프레임워크란?</strong>
누군가 정한 규칙에 따라 도구를 사용하고 있다.
프레임워크가 우리를 제어한다. 규칙을 바꿀수 없으므로 규칙을 따라야한다.

<strong>API 란?</strong>
프로그램끼리 소툥해주도록 연결해주는것. 
사용자는 API 가 제공하는 기능의 결과만 보지 원리까지 알 필요는 없다. 

<strong>도메인 이란?</strong>
홈페이지 숫자 주소 123.456.778 보다 쉬운 naver.com 같은 이름으로 기억하기 쉽도록 만든 주소. 

<li>오늘 읽은 소감은? 떠오르는 생각을 가볍게 적어보세요</li>
지금까지 무엇을 공부할지 체크리스트를 정해서 문법만 익혔는데, 어떤 토이 프로젝트를 완성할지 체크리스트를 작성해서 꾸준히 완성해 나가야겠다. 

<li>궁금한 내용이 있거나, 잘 이해되지 않는 내용이 있다면 적어보세요.</li>
제이쿼리 문접, 장고와 스프링

---

<h3>Today I Learn 4차</h3>
2장. 코딩별 안내서 - 웹기술편
2023. 01. 18

<li>오늘 TIL 3줄 요약</li>

웹 브라우저가 최신 기술을 지원한다는것은 중요하다.
서버는 그냥 항상 켜져있는 컴퓨터다.

<li>책에서 기억하고 싶은 내용을 써보세요.</li>

<strong>HTTP란?</strong>
인터넷에서 사용자와 서버가 정보를 주고받기 위한 일종의 규칙.
데아토룰 다 보내면 연결상태가 끊기기 때문에 쿠키가 필요

<strong>쿠키란?</strong>
서버가 우리를 기억하게 하는것.
웹사이트 접속때마다 서버에 전송되어 서버의 기억을 되살리는 역할.

<strong>서버 란?</strong>
웹사이트주소를 입력하기 기다렸다가 해당 주소가 입력되면 그에 맞는 웹주소를 꺼내서 주는 코드.

<strong>서버리스 란?</strong>
우리가 직접관리하지 않는 서버.
함수가 필요할때 깨워서 요청한 작업을 수행한다.
그래서 비용 면에서 큰 장점이 있다. 등록된 함수가 실행된 만큼만 돈을 내면 된다.
단점은 잠에서 깰때 시간이 필요하며 서비스를 옮기기 쉽지않다.

<strong>도메인 이란?</strong>
홈페이지 숫자 주소 123.456.778 보다 쉬운 naver.com 같은 이름으로 기억하기 쉽도록 만든 주소. 

<li>오늘 읽은 소감은? 떠오르는 생각을 가볍게 적어보세요</li>
서버터졌다는데 그냥 컴퓨터 인터넷 끊겼다는 말이었구나!

<li>궁금한 내용이 있거나, 잘 이해되지 않는 내용이 있다면 적어보세요.</li>
데브옵스,,, 다들 풀스택만 쓰지 데브옵스란 용어를 안써서 생소했다.

---

<h3>Today I Learn 5차</h3>
3장. 코딩별 안내서 - 컴퓨터 공학편
2023. 01. 19

<li>오늘 TIL 3줄 요약</li>

데이터는 원유와 같아서 없으면 아무것도 만들수없다.
무료 서비스의 대가로 그들은 우리 데이터를 수집힌다. 
알고리즘과 자료구조를 배워야 하는 이유 : 효율적인 지시사항의 나열과 올바른 데이터 보관 방법으로 컴퓨터가 빨라지도록 할수있다.

<li>책에서 기억하고 싶은 내용을 써보세요.</li>

<strong>알고리즘 이란?</strong>
컴퓨터에게 내리는 지시사항을 나열한것.
효율적인 지시사항을 나열해서 컴퓨터가 속도가 빨라지도록 한다.

<strong>자료구조 란?</strong>
수집한 데이터를 관리하는것.
올바른 데이터 보관 방법으로 프로그램 속도가 빨라지게 할수있다.

<li>오늘 읽은 소감은? 떠오르는 생각을 가볍게 적어보세요</li>
일단 알고리즘의 속도는 수행작업을 몇단계로 하느냐에 결정된다까지만 이해하자...

<li>궁금한 내용이 있거나, 잘 이해되지 않는 내용이 있다면 적어보세요.</li>
알고리즘 배열은 아직 이해하기 어렵다. Big-O ?

---

<h3>Today I Learn 6차</h3>
3장. 코딩별 안내서 - 컴퓨터 공학편
2023. 01. 20

<li>오늘 TIL 3줄 요약</li>
자신이 만든 프로그램은 사람들에게 영향을 준테니 완벽하게 책임을 져라.
코드를 살아있는 생명체처럼 대하라. 방치하면 죽는거와 똑같다. 

<li>책에서 기억하고 싶은 내용을 써보세요.</li>

<strong>클린코드란?</strong>
설명이 필요없는 코드. 코드를 읽기만 해도 이 코드가 무슨 일을 하는지, 어떤것을 의미하는지 물어볼 필요도 없이 스르륵 이해되는 그런 코드.

<strong>클린코드 꿀팁</strong>
1. 함수 이름을 짐작이 가능하도록 이름 지어라. 
2. 함수 이름은 무조건 동사로 지어라.  함수의 액션은 1개여야 한다. 하나의 함수는 1개의 역할만 하는것이 좋다. 그래야 함수가 하는 역할을 명확하게 지정할수 있다. 
3. 매개변수는 3개 이하로 써라. function 매개변수(1,2,3)
4. 불린값을 인자로 쓰지마라. 불린은 참,거짓에 따라 2가지 일을 처리해야하는데 함수는 1가지 일만 잘해야 한다.
5. 나 혼자만 알아볼수있는 축약어를 쓰지마라. 
6. 마지막에는 항상 위와같이 클린하게 코드 다듬는 작업을 해라.

<li>오늘 읽은 소감은? 떠오르는 생각을 가볍게 적어보세요</li>
코드도 살아있는 생명체처럼 죽지않도록 애정을 가지고 관리하고 개선해 나가야하다니...물줘야겠다.

<li>궁금한 내용이 있거나, 잘 이해되지 않는 내용이 있다면 적어보세요.</li>
스택과 큐 아직 알고싶지 않다 ㅎ

---

<h3>Today I Learn 7차</h3>
4장. 코딩별 안내서 - 컴퓨터 공학편 2
2023. 01. 25

<li>오늘 TIL 3줄 요약</li>
기술에는 좋고 나쁨이 없다. 용도에 맞게 쓰기만 한다면.
맥북을 살 때 무엇보다 반드시 개발자가 되어서 본전을 뽑겠다는 의지를 가지고 나에게 투자했다고 생각하자.

<li>책에서 기억하고 싶은 내용을 써보세요.</li>

<strong>SQL?</strong>
데이터베이스를 다루는 언어. 
데이터베이스를 관리해주는 DBMS와 대화하기 위한 언어.
TABLE 표에 데이터를 행과 열로 저장. -> 그래서 매우 정적

<strong>데이터베이스?</strong>
데이터를 보관하는 창고.

<strong>DBMS?</strong>
데이터를 정리하거나 처리하는 능력.

<strong>NoSQL?</strong>
데이터를 대괄호와 중괄호로 구분해서 저장.
[{"id":1, "name": jelly, "price":3000}, {"id":1, "name": jelly, "price":3000}]
-> 원하는 모양으로 어떠한 종류의 데이ㅓ라도 저장할수있다.

<strong>깃?</strong>
파일의 이력을 관리하는 프로그램.

<strong>깃헙?</strong>
어디서든 접근할수있는 곳에 저장해서 공유할수있는 저장소.

<strong>시멘틱 버저닝</strong>
숫자 3개를 사용하는 버전 표기 방식. 
version 2.3.25
첫번째 숫자 : 프로그램에 엄청나게 큰 변화가 있을때.
두번째 숫자 : 마이너한 업데이트.
마지막 숫자 : 패치나 버그 수정.

<li>오늘 읽은 소감은? 떠오르는 생각을 가볍게 적어보세요</li>
SQL과 NoSQL 이름이 비슷하게 같은 계열이라 생각되는데, 테이블표 형식(SQL)과 행과열 형식(NoSQL)의 차이가 있다.

<li>궁금한 내용이 있거나, 잘 이해되지 않는 내용이 있다면 적어보세요.</li>
비밀번호를 설정할때 키를 쓰거나 해시함수를 쓸수 있는데 그럼 키나 레인보우테이블(해시함수와 연결값의 표)을 털렸을때 위험하므로 무작위 테스트인 솔트를 사용한다.

---

<h3>Today I Learn 8차</h3>
4장. 코딩별 안내서 - 컴퓨터 공학편 2
2023. 01. 25

<li>오늘 TIL 3줄 요약</li>
갈때까지 가봅시다. 남는게 체력인데.

<li>책에서 기억하고 싶은 내용을 써보세요.</li>

<strong>프로그래밍 패러다임</strong>
프로그래밍을 하는 사고의 틀.
1. 절차 지향 프로그래밍
2. 객체 지향 프로그래밍
3. 함수형 프로그래밍

<strong>객체지향 프로그래밍</strong>
1. 클레스 개념
공장에 넣어서 꺼내쓴다.
class Jelly {arrat DATA};
const haribo = new Jelly("1", 2, 3);
2. 상속 개념
기본 속성은 다 가지면서도 자신만의 속성을 추가할수있다.
<underline>class</underline> haribo <underline>extends</underline> Jelly{}
haribo는 Jelly 에게 상속된다.

<strong>함수형 프로그래밍</strong>
선언형 콘셉트를 유지하여 버그가 발생하기 어려운 구조가 장점.

<strong>선언형 프로그래밍?</strong>
원하는 결과값을 선언 -> 결과중심으로 실수가 적고 알아보기 쉬움.

<strong>명령형 프로그래밍?</strong>
원하는 결과값에 어떻게 도달하는지 선언 -> 실수할 가능성이 큼.

<li>오늘 읽은 소감은? 떠오르는 생각을 가볍게 적어보세요</li>
객체지향이던 함수형이던 잘 모르겠고 그냥 반복되는 코드를 줄여주기 위해 나온것이라 생각하자. ㅎ

<li>궁금한 내용이 있거나, 잘 이해되지 않는 내용이 있다면 적어보세요.</li>
항상 공식 문서에 객체지향 프로그래밍이라고 대문짝만하게 써있는데 무슨말인지 아직도 이해가 안됐는데, 여전히 이해는 안된다. ㅎ

---

<h3>Today I Learn 9차</h3>
5장. 코딩별 안내서 - 최신 기술 편
2023. 01. 27

<li>오늘 TIL 3줄 요약</li>

<li>책에서 기억하고 싶은 내용을 써보세요.</li>
머신러닝을 배울 수 있는 가장 쉬운 방법은 파이썬을 이용하는 것이고, 인공지능을 개발하는 도구 "덴서플로"를 공부해 봐.
REST API 는 설계 철학 그 이상도 이하도 아니다. 

<strong>머신러닝</strong>
인공지능을 학습시키는 방법

<strong>딥러닝</strong>
머신러닝을 달성하기 위한 방법. 여러겹으로 층을 깊이(deep) 학습. 생물의 신경망에서 영감을 얻음.

<strong>REST API</strong>
동사를 사용하면 안된다. (동사 단어 혼용 방지)
HTTP 메서드로 갈림길을 만들어 사용.
주로 GET POST PUT DELETE 이 있는데 URL을 단순하게 바꿔줌. GET/movies/inception/actors 

<strong>HTTP 메서드</strong>
메소드는 총 8개가 존재하며 아래와 같습니다.
GET: 서버로 부터 데이터를 취득. 주로 데이터를 읽거나(Read) 검색(Retrieve)할 때에 사용되는 메소드이다. 
POST: 서버에 데이터를 추가, 작성 등. 주로 새로운 리소스를 생성(create)할 때 사용된다.
PUT: 서버의 데이터를 갱신, 작성 등. 리소스를 생성 / 업데이트하기 위해 서버로 데이터를 보내는 데 사용됩니다.
DELETE: 서버의 데이터를 삭제. 지정된 리소스를 삭제합니다.
HEAD: 서버 리소스의 헤더(메타 데이터의 취득)
OPTIONS: 리소스가 지원하고 있는 메소드의 취득
PATCH: 리소스의 일부분을 수정
CONNECT: 프록시 동작의 터널 접속을 변경

<strong>POST vs PUT</strong>
POST와 PUT은 구분해서 사용해야한다. POST는 새로운 데이터를 계속 생성하기 때문에 요청시마다 데이터를 생성하지만, PUT은 사용자가 데이터를 지정하고 수정하는 것이기 때문에 같은 요청을 계속하더라도 데이터가 계속 생성되지는 않는다.

<li>오늘 읽은 소감은? 떠오르는 생각을 가볍게 적어보세요</li>
REST API (GET,POST,PUT,DELETE) 의 장점 HTTP 메서드가 코드 앞에서 깔끔하게 분류해서 코드를 단순하게 해준다.

<li>궁금한 내용이 있거나, 잘 이해되지 않는 내용이 있다면 적어보세요.</li>
REST API를 알면 JWT를 배워야 하는데 아직도 모르겠다.

---
